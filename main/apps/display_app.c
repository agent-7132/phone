...
lv_obj_t *display_app_create(void)
{
    ...
    lv_obj_set_size(color_rect, 300, 300);
    lv_obj_set_style_bg_color(color_rect, test_colors[color_index], 0);
    lv_obj_set_style_border_width(color_rect, 2, 0);
    lv_obj_set_style_border_color(color_rect, lv_color_hex(0xFFFFFF), 0);
    lv_obj_set_style_radius(color_rect, 10, 0);
    lv_obj_align(color_rect, LV_ALIGN_CENTER, 0, -20);

    color_label = lv_label_create(scr);
    lv_label_set_text_fmt(color_label, "Color: %s", color_names[color_index]);
    ...
    lv_obj_align(color_label, LV_ALIGN_CENTER, 0, 170);

    lv_obj_t *next_btn = lv_btn_create(scr);
    lv_obj_set_size(next_btn, 200, 50);
    lv_obj_set_style_bg_color(next_btn, lv_color_hex(0x4a4a6a), 0);
    lv_obj_set_style_bg_color(next_btn, lv_color_hex(0x5a5a8a), LV_STATE_PRESSED);
    ...
    ESP_LOGI(TAG, "Display app created");
    return scr;
}
